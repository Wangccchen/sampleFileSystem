...
//功能函数声明
void createAndAssignDir(struct file_directory *directory, const char *filename, const char *extension, uint16_t inodeNumber, char *stdby);
//创建目录项的函数，分别给file_directory数据结构每个字段赋值
void createAndAssignDir(struct file_directory *directory, const char *filename, const char *extension, uint16_t inodeNumber, char *stdby)
{
    // 复制文件名和扩展名到目录项中
    strncpy(directory->dirFile, filename, MAX_FILENAME);
    strncpy(directory->dirFile + MAX_FILENAME, extension, MAX_EXTENSION);

    // 设置Inode号，以2字节的形式存储
    memcpy(directory->dirFile + MAX_FILENAME + MAX_EXTENSION, &inodeNumber, sizeof(inodeNumber));

    //备用字段
    strncpy(&ddir->dirFile + 13, stdby, 3);
}
...
int main()
{
    ...
    //进行初始化的设置
    dirInode->st_mode = S_IFDIR | S_IRUSR | S_IWUSR | S_IXUSR; //表示为目录,并且可读写执行
